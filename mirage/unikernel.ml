...
module Make (Stack : Tcpip.Stack.V4V6) = struct
  ...
  let store = Mirageio.Site.connect ()
  ...
  module Last_modified = struct
    ...
    let not_modified request store key =
      let* last_modified =
        let* s = store in
        Mirageio.Site.last_modified s key
      in
  ...
  let serve_static reqd path =
    let key = Mirage_kv.Key.v path in
    let* store = store in
    let* result = Mirageio.Site.get store key in
    match result with
    | Ok data ->
        let request = H1.Reqd.request reqd in
        let* modified = Last_modified.not_modified request (Lwt.return store) key in
        (match modified with
        | Some (_, true) ->
    ...
    | Error _ ->
        ...
        let* result = Mirageio.Site.get store (Mirage_kv.Key.v index) in
        (match result with
        | Ok data ->
            respond_with reqd `OK
        ...
        | Error _ ->
            let* result =
              Mirageio.Site.get store (Mirage_kv.Key.v "404.html")
            in
            let body =
  ...
  let handle_request reqd =
    ...
    | None ->
        ...
        | None ->
            ...
            in
            serve_static reqd kv_path)

  let request_handler _flow _dst reqd =
    Lwt.async (fun () -> handle_request reqd)
