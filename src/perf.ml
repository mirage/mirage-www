 ...
 <:html<
   <p>This page describes some of the measurements we have done <i>during</i> development of Mirage, and descriptions of some of the problems we encountered and how they were fixed. We use DNS serving performance as the first measure of I/O performance, for several reasons:</p>
    <ul>
     ...
     <li><p>The [queryperf] test suite provides realistic test data that we can also run against other servers.</p></li>
  ...
  <ul>
    ...
    <li><p><a href="http://github.com/avsm/mirage-perf/tree/master/dns/bin/">bin/generate.sh</a> runs <tt>queryperf</tt> to create the test zone files, and compiles the Mirage UNIX and kernel binaries for different result sizes</p></li>
