We've just released [Mirage 1.1.0](https://github.com/ocaml/opam-repository/pull/1655) into OPAM.  Once the
...
signature.  All of the module types used in Mirage can be browsed in [one
source file](https://github.com/mirage/mirage/blob/1.1.0/types/V1.mli).

In Mirage 1.1.0, [Thomas Gazagnaire](http://gazagnaire.org/) implemented a
...
With Mirage 1.1.0, this is all a thing of the past.  All the libraries can
...
non-Mirage applications, and the `tcpip` OPAM package installs pre-applied Unix
...
* [xen-evtchn](https://github.com/xapi-project/ocaml-evtchn) for the event notification mechanism. There are a couple of wiki posts that explain how [event channels](/wiki/xen-events) and [suspend/resume](/wiki/xen-suspend) work in Mirage/Xen guests.
...
explains how you can synthesize your own virtual disk backends using Mirage.  Xen uses a [split device](https://www.usenix.org/legacy/event/usenix05/tech/general/full_papers/short_papers/warfield/warfield.pdf) model,
and now Mirage lets us build *backend* device drivers that service VMs as well as the frontends!

Last, but not least, [Thomas Gazagnaire](http://gazagnaire.org) has been building a brand new storage system for Mirage guests that uses git-style branches under the hood to help coordinate clusters of unikernels.  We'll talk about how this works in a future update, but there are some cool libraries and prototypes available on OPAM for the curious.
