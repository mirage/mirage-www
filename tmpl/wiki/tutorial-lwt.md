...
##Challenge 2: Echo server

Write an echo server that reads from a dummy input generator and, for each line it reads, writes it to the console. The server should stop after reading 10 lines. Here is a basic dummy input generator:
...
By the way, the reason `async` and `catch` take functions rather than plain threads is so they can catch OCaml exceptions (not just failed threads). Otherwise, if we had `Lwt.catch (fn data) ...` and `fn data` used `raise` then the exception would occur before `Lwt.catch` ran, and it wouldn't be able to handle it.
...
If a bug is detected, you should raise an exception. In threaded code you should use `Lwt.fail`, although Lwt should catch exceptions and turn them into failures automatically if you forget.
...
In general, if you allocate a resource that won't be automatically freed by the GC then you should use `Lwt.finalize` to ensure it is cleaned up whether the function using it succeeds or not:
...
Note: this example also shows a new convenience operator provided by Lwt. `>|=` ("map") is similar to `>>=` but automatically wraps the result of the function you provide with `return`. It's used here because `cancel` is synchronous (it doesn't return a thread).
