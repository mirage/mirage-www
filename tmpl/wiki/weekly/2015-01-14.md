...
would prefer to merge quickly.  The functor inline patch from 4.02.1 will have
big performance boost but we should clearly state that we're not dropping
support for 4.01 though.
...
speed of normal TLS?). Issue now is how to merge without breaking things as
...
can just use [Open Types][opentypes].  The nice thing with current patch is
