...
Apart from talking to old and new friends face-to-face, which we haven't done for some time, we also worked on various projects to improve the MirageOS ecosystem. We also had very interesting talks about the opam repository CI and opam health check, Lwt, BGP, monitoring, ... Below is a collection of projects that wouldn't come to existance without the retreat. Please excuse if there's some project missing.

We did a long [dragon dreaming](https://dragondreaming.org/) session to figure out the ideas and dreams for the future of MirageOS. Apart from bare-metal and MirageOS as hypervisor, we also dreamed about running your own digital infrastructure with MirageOS (with increased decentralization and security).
...
#### Integrating a Verified File System

[FSCQ](https://github.com/mit-pdos/fscq) is a formally verified file system developed in the Coq proof assistant. So far MirageOS has a variety of [chamelon](https://github.com/yomimono/chamelon/) (LittleFS) [tar](https://github.com/mirage/ocaml-tar/) (append-only), and [fat](https://github.com/mirage/ocaml-fat/) (FAT16, barely used), [one file FS](https://github.com/reynir/oneffs) (supporting only one file).
At the retreat, we [first repaired some FSCQ proofs](https://github.com/mit-pdos/fscq/pull/17) to work with recent Coq releases, and then added [OCaml wrapper code and revised the OCaml extraction](https://github.com/mit-pdos/fscq/pull/18). The current state is that all the tests are passing, but we need to improve the performance of some operations. FSCQ was developed with lazyness in mind targetting Haskell, and uses lots of lists. In OCaml with strict evaluation the list operations are expensive.
